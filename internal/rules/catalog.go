...
var catalog = []*Rule{
	...
	},
	{
		...
		// setzt ihn per Spaltentransposition wieder zusammen: Zeichen r, r+N,
		// r+2N … einer Zeile werden an Spalte r gehängt. Genau diese Lesung -
		// ein Array-Element bekommt ein Zeichen aus $s[$r + $i * $n] angehängt -
	...
	{
		ID:          "htaccess.cgi_handler",
		Severity:    report.SeverityHigh,
		Description: "macht eine fremde Endung über CGI ausführbar",
		// Die ALFA-Shell legt ein .htaccess an, das .alfa als CGI-Skript
	...
	},
	{
		...
		Severity:    report.SeverityHigh,
		Description: "schaltet die Web Application Firewall ab",
		// „Generated By Sole Sad and Invisible“ steht über einem Block, der
		// mod_security stilllegt (Sec------Engine Off). Eine abgelegte Datei
		// deckt sich damit selbst. Ehrlicher Code entfernt höchstens eine
	...
	// ------------------------------------------------------------ by place
	{
		...
		// ALFA_DATA/alfacgiapi and gives them the extension .alfa. Neither
		// occurs in honest software, so the location is the whole finding -
		// the file need not be PHP, and the shell scripts here are not. Not
		// AutoSafe, as every location rule: the move waits for a human,
		// although a directory by this name has no innocent reading.
		RawOnly:   true,
		PathMatch: rx(`(?i)(?:/(?:ALFA_DATA|alfacgiapi)/|\.alfa$)`),
		Match:     rx(`(?s)^`),
	},
	{
		ID:          "php.in_uploads",
		...
		Exts:        phpExts,
		// The question is what the web server would run, and the server
